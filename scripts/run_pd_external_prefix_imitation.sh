...
export EXPERIMENT_PROFILE="${EXPERIMENT_PROFILE:-balanced_dual_pressure}"
...
export MAX_ROWS_TO_SCAN="${MAX_ROWS_TO_SCAN:-}"
export NUM_SESSIONS="${NUM_SESSIONS:-}"
export REUSE_TURNS_PER_SESSION="${REUSE_TURNS_PER_SESSION:-}"
export SEED_PROMPT_TOKENS="${SEED_PROMPT_TOKENS:-}"
export APPEND_TOKENS="${APPEND_TOKENS:-}"
export DECODE_TOKENS="${DECODE_TOKENS:-}"
export POST_REQUEST_SETTLE_MS="${POST_REQUEST_SETTLE_MS:-}"
export SLEEP_BETWEEN_GROUPS_MS="${SLEEP_BETWEEN_GROUPS_MS:-0}"
export GROUP_CONCURRENCY="${GROUP_CONCURRENCY:-}"
...
case "$EXPERIMENT_PROFILE" in
  prefill_max)
    ...
    : "${GROUP_CONCURRENCY:=$NUM_SESSIONS}"
    ;;
  ...
  decode_heavy)
    ...
    ;;
  *)
    echo "unknown EXPERIMENT_PROFILE=$EXPERIMENT_PROFILE"
    echo "supported profiles: prefill_max, balanced_dual_pressure, decode_heavy"
    exit 1
    ;;
esac
...
echo "NUM_SESSIONS=$NUM_SESSIONS GROUP_CONCURRENCY=$GROUP_CONCURRENCY MAX_NUM_SEQS=$MAX_NUM_SEQS"
echo "SEED_PROMPT_TOKENS=$SEED_PROMPT_TOKENS APPEND_TOKENS=$APPEND_TOKENS DECODE_TOKENS=$DECODE_TOKENS"
echo "POST_REQUEST_SETTLE_MS=$POST_REQUEST_SETTLE_MS GPU_MEMORY_UTILIZATION=$GPU_MEMORY_UTILIZATION"
